...
func (wfs *WFS) handleRenameResponse(ctx context.Context, resp *filer_pb.StreamRenameEntryResponse) error {
	...
	if resp.EventNotification.NewEntry != nil {
		if err := wfs.applyLocalMetadataEvent(ctx, metadataEventFromRenameResponse(resp)); err != nil {
			...
			wfs.invalidateDirectoryCacheWithReason(util.FullPath(resp.Directory), "rename_create_fragment_apply_failed")
			if resp.EventNotification.NewParentPath != "" {
				wfs.invalidateDirectoryCacheWithReason(util.FullPath(resp.EventNotification.NewParentPath), "rename_create_fragment_apply_failed")
			}
		}
		...
		if err := wfs.applyLocalMetadataEvent(ctx, metadataEventFromRenameResponse(resp)); err != nil {
			...
			wfs.invalidateDirectoryCacheWithReason(util.FullPath(resp.Directory), "rename_delete_fragment_apply_failed")
		}
	}
